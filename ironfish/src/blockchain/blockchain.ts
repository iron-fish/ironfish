...
export class Blockchain {
  ...
  orphans: LRU<BlockHash, NetworkBlockHeader>
  ...
  onConnectBlock = new Event<[block: LocalBlock, tx?: IDatabaseTransaction]>()
  // When ever a block is removed from the heaviest chain, trees have not been updated yet
  onDisconnectBlock = new Event<[block: LocalBlock, tx?: IDatabaseTransaction]>()
  // When ever a block is added to a fork
  onForkBlock = new Event<[block: LocalBlock, tx?: IDatabaseTransaction]>()
  ...
  private _latest: NetworkBlockHeader | null = null
  get latest(): NetworkBlockHeader {
    Assert.isNotNull(
      this._latest,
  ...
  set latest(newLatest: NetworkBlockHeader) {
   ...
   */
  async findFork(
    headerA: NetworkBlockHeader | Block,
    headerB: NetworkBlockHeader | Block,
    tx?: IDatabaseTransaction,
  ): Promise<{
    fork: NetworkBlockHeader
    isLinear: boolean
  }> {
   ...
   * */
  async *iterateTo(
    start: NetworkBlockHeader,
    end?: NetworkBlockHeader,
    tx?: IDatabaseTransaction,
    reachable = true,
  ...
  async *iterateToHashes(
    ...
    let current: BlockHash | null = start.hash
    let last: BlockHash | null = null
  ...
  async *iterateFrom(
    ...
    let current: LocalBlockHeader | null = start
  ...
  isInvalid(headerOrHash: NetworkBlockHeader | BlockHash): VerificationResultReason | null {
  ...
  async disconnect(block: LocalBlock, tx: IDatabaseTransaction): Promise<void> {
    Assert.isTrue(
  ...
  addOrphan(header: NetworkBlockHeader): void {
  ...
  async isHeadChain(header: NetworkBlockHeader, tx?: IDatabaseTransaction): Promise<boolean> {
  ...
  async getNext(header: NetworkBlockHeader, tx?: IDatabaseTransaction): Promise<NetworkBlockHeader | null> {
