 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 describe('Blockchain', () => {
   ...
   describe('asset updates', () => {
     ...
     async function burnAsset(node: IronfishNode, account: Account, sequence: number, asset: Asset, value: bigint, noteToBurn: NoteEncrypted): Promise<Block> {
       const transaction = await useRawTxFixture(
         node.chain, 
         node.workerPool, 
         account, [noteToBurn], [], [], [{asset, value}])
