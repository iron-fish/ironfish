...
export class BlockchainDB {
  ...
  async hasBlockHashesAtSequence(
    sequence: number,
    tx?: IDatabaseTransaction,
  ): Promise<boolean> {
  ...
  async isHeadChain(header: BlockHeader, tx?: IDatabaseTransaction): Promise<boolean> {
    const hash = await this.getBlockHashAtSequence(header.sequence, tx)
  ...
  async deleteHashToNextHash(hash: Buffer, tx?: IDatabaseTransaction): Promise<void> {
    return this.hashToNextHash.del(hash, tx)
  }

  async deleteSequenceToNextHash(sequence: number, tx?: IDatabaseTransaction): Promise<void> {
