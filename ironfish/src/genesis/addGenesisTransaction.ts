...
export async function addGenesisTransaction(
  ...
  for (const { transaction, initialNoteIndex } of genesisTransactions) {
    ...
    for (const encryptedNote of transaction.notes) {
      ...
      if (await node.chain.blockchainDb.hasNullifier(nullifier)) {
        continue
      }
      ...
      witness = await node.chain.blockchainDb.getNoteWitness(initialNoteIndex + noteIndex)
  ...
  await node.chain.blockchainDb.addNotesBatch(postedTransaction.notes)

  // Generate a new block header for the new genesis block
  const noteCommitment = await node.chain.blockchainDb.getNotesRootHash()
  const noteSize = await node.chain.blockchainDb.getNotesSize()
