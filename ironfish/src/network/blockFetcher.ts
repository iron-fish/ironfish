...
type BlockState =
  ...
  | {
      action: 'PROCESSING_ASSEMBLED_BLOCK'
      block: Block
    }
...
export class BlockFetcher {
  ...
  async hashReceived(hash: BlockHash, peer: Peer): Promise<void> {
  ...
  blockTransactionsReceived(hash: BlockHash): SerializedCompactBlock | null {
    const currentState = this.pending.get(hash)
    ...
    const block = currentState.block
  ...
  /**
   * Called when a block has been received from the network
   * but has not yet been processed (validated etc.) */
  receivedBlock(hash: BlockHash): void {
    const currentState = this.pending.get(hash)
  ...
  assembledBlock(block: Block): void {
    const currentState = this.pending.get(block.header.hash)

    if (currentState) {
      this.cleanupCallbacks(currentState)
    }

    this.pending.set(block.header.hash, {
      action: 'PROCESSING_ASSEMBLED_BLOCK',
      block,
    })
  }
  ...
  getAssembledBlock(blockHash: BlockHash): Block | null {
    const currentState = this.pending.get(blockHash)

    if (!currentState || currentState.action !== 'PROCESSING_ASSEMBLED_BLOCK') {
      return null
    }
