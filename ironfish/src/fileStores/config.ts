...
export const ConfigOptionsSchema: yup.ObjectSchema<Partial<ConfigOptions>> = yup
  ...
  // Already got a pareser to validate
  logLevel: yup.string(),
  logPrefix: yup.string().matches(RegExp('^([^%]*%(time|level|tag)%[^%]*)*$')),
  blockGraffiti: yup.string(),
  nodeName: yup.string(),
  nodeWorkers: yup.number().min(-1),
  nodeWorkersMax: yup.number(),
  p2pSimulateLatency: yup.number(),
  peerPort: yup.number(),
  rpcTcpHost: yup.string(),
  rpcTcpPort: yup.number(),
  rpcTcpSecure: yup.boolean(),
  tlsKeyPath: yup.string(),
  tlsCertPath: yup.string(),
  maxPeers: yup.number(),
  minPeers: yup.number(),
  targetPeers: yup.number(),
  telemetryApi: yup.string(),
  accountName: yup.string(),
  generateNewIdentity: yup.boolean(),
  defaultTransactionExpirationSequenceDelta: yup.number(),
  blocksPerMessage: yup.number(),
  minerBatchSize: yup.number(),
  minimumBlockConfirmations: yup.number(),
  poolName: yup.string(),
  poolAccountName: yup.string(),
  poolBanning: yup.boolean(),
  poolBalancePercentPayout: yup.number(),
  poolHost: yup.string(),
  poolPort: yup.number(),
  poolDifficulty: yup.string(),
  poolAttemptPayoutInterval: yup.number(),
  poolSuccessfulPayoutInterval: yup.number(),
  poolStatusNotificationInterval: yup.number(),
  poolRecentShareCutoff: yup.number(),
  poolDiscordWebhook: yup.string(),
  poolMaxConnectionsPerIp: yup.number(),
  poolLarkWebhook: yup.string(),
  jsonLogs: yup.boolean(),
  explorerBlocksUrl: yup.string(),
  explorerTransactionsUrl: yup.string(),
  })
  .defined()
