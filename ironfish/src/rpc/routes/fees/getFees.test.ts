...
import { useBlockWithTx, useMinerBlockFixture, useTxFixture } from '../../../testUtilities'
...
describe('Route chain.getFees', () => {
  ...
  it('responds with fees', async () => {
    ...
    expect(response.content).toMatchObject({
      startBlock: 2,
      endBlock: 3,
      p25: Number(transaction.fee()),
      p50: Number(transaction.fee()),
      p75: Number(transaction.fee()),
    })
  })
})
