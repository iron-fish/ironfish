  ...
  it('should call retrieve correct block/transaction info', async () => {
    ...
    const raw = await routeTest.wallet.createEvmTransaction({
      evm: {
        ...
        publicIron: BigInt(0),
        to: Buffer.from('a'.repeat(40), 'hex'),
        data: Buffer.alloc(0),
      },
    })
    ...
    const result = await routeTest.client.eth.getTransactionReceipt(
      Buffer.from(ethHash).toString('hex'),
    )

    expect(result.status).toEqual(200)
    expect(result.content).toMatchObject({
      transactionHash: EthUtils.prefix0x(Buffer.from(ethHash).toString('hex')),
      transactionIndex: '0x1',
      blockHash: EthUtils.prefix0x(block1.header.hash.toString('hex')),
      blockNumber: '0x2',
      from: EthUtils.prefix0x(evmSenderAddress.toString()),
      to: EthUtils.prefix0x('a'.repeat(40)),
      cumulativeGasUsed: '0x5208',
      effectiveGasPrice: '0x0',
      gasUsed: '0x0',
      contractAddress: null,
      logs: [],
      logsBloom: EthUtils.prefix0x('0'.repeat(512)),
      type: '0x0',
      status: '0x1',
    })
  })
