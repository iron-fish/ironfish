...
export const ExportAccountRequestBase58Schema: yup.ObjectSchema<ExportAccountBase58Request> =
  yup
    .object({
...
export const ExportAccountResponseBase58Schema: yup.ObjectSchema<ExportAccountBase58Response> =
  yup
    .object({
      account: yup
        .object({
...
router.register<typeof ExportAccountRequestBase58Schema, ExportAccountBase58Response>(
  ...
  (request, node): void => {
    ...
    account.spendingKey = bs58safe.encode(Buffer.from(account.spendingKey, 'hex'))
    account.incomingViewKey = bs58safe.encode(Buffer.from(account.incomingViewKey, 'hex'))
    account.outgoingViewKey = bs58safe.encode(Buffer.from(account.outgoingViewKey, 'hex'))
