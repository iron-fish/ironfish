...
export class TestNode {
  ...
  attachListeners(p: ChildProcessWithoutNullStreams, procName: string): void {
    p.stdout.on('data', (data) => {
      ...
      this.logger.log(`[${this.name}:${procName}:stdout]: ${data}`)
      try {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const data2: Record<string, Loggable> = JSON.parse(stringData)
        this.logger.log(`[${this.name}:${procName}]`, data2)
      } catch (e) {
        this.logger.log(`[${this.name}:${procName}]`, data)
      }
    })

    p.stderr.on('data', (data) => {
      this.logger.log(`[${this.name}:${procName}:stderr]: ${data}`)
    })

    p.on('error', (error: Error) => {
      this.logger.log(`[${this.name}:${procName}:error]: ${error.message}`)
    })
    ...
    p.on('exit', (code, signal) => {
      ...
      this.logger.log(`[${this.name}:${procName}:error]:spawn`, {
        code,
