...
use ironfish::xchacha20poly1305::{self, XChaCha20Poly1305Key, SALT_LENGTH, XNONCE_LENGTH as XNONCE_SIZE};
use napi::{
...
impl NativeXChaCha20Poly1305Key {
    ...
    pub fn from_parts(
        ...
        let mut salt_bytes = [0; SALT_LENGTH];
        salt_bytes.clone_from_slice(&salt_vec[0..SALT_LENGTH]);
        ...
        let key = XChaCha20Poly1305Key::from_parts(
            passphrase.as_bytes(), 
            salt_bytes, 
            nonce_bytes).map_err(to_napi_err)?;

        Ok(NativeXChaCha20Poly1305Key { key })
    }

    #[napi]
    pub fn derive_key(&self, salt: JsBuffer, nonce: JsBuffer) -> Result<NativeXChaCha20Poly1305Key> {
        let salt_buffer = salt.into_value()?;
        let salt_vec = salt_buffer.as_ref();
        let mut salt_bytes = [0; SALT_LENGTH];
        salt_bytes.clone_from_slice(&salt_vec[0..SALT_LENGTH]);
    ...
    pub fn encrypt(&self, plaintext: JsBuffer) -> Result<Buffer> {
        ...
        let result = xchacha20poly1305::encrypt(plaintext_bytes.as_ref(), &self.key)
            .map_err(to_napi_err)?;
    ...
    pub fn decrypt(&self, ciphertext: JsBuffer) -> Result<Buffer> {
        ...
        let result =
            xchacha20poly1305::decrypt(byte_vec.to_vec(), &self.key).map_err(to_napi_err)?;
